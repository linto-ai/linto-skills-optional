...
module.exports = function (RED) {
  ...
  function loadLanguage(language) {
    if (language === undefined) language = process.env.DEFAULT_LANGUAGE;
    lintoResponse = require("./locales/" + language + "/friendly").friendly
      .response;
  }
  ...
  function intentDetection(input) {
    return (
      (input.conversationData !== undefined &&
        input.conversationData.intent === intent.keys.status) ||
      intent.keys.hasOwnProperty(input.nlu.intent)
    );
  }

  function outputDetection(input) {
    // Is conversational
    if (input.conversationData !== undefined) {
      let say = lintoResponse.status_unk;
      if (input.nlu.entitiesNumber !== 1) say = lintoResponse.status_unk;
